...
rule compute_signatures_splicing:
    input:
        metadata = os.path.join(PREP_DIR,'metadata','Urbanski2022.tsv.gz'),
        splicing = os.path.join(PREP_DIR,'event_psi','Urbanski2022-EX.tsv.gz'),
    output:
        signatures = os.path.join(PREP_DIR,"signatures","Urbanski2022-EX.tsv.gz")
    run:
        import pandas as pd
...
rule compute_signatures_genexpr:
    input:
        ...
        genexpr = os.path.join(PREP_DIR,'genexpr_tpm','Urbanski2022.tsv.gz'),
    output:
        signatures = os.path.join(PREP_DIR,"signatures","Urbanski2022-genexpr_tpm.tsv.gz")
    run:
        import pandas as pd
        ...
        # subset
        common_samples = set(metadata["sampleID"]).intersection(genexpr.columns)
        metadata = metadata.loc[metadata["sampleID"].isin(common_samples)].copy()
        genexpr = genexpr[common_samples].copy()
